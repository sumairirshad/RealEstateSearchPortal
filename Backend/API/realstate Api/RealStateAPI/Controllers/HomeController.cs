...
namespace RealStateAPI.Controllers
{
    ...
    public class HomeController : ControllerBase
    {
        ...
        [HttpPost("fetchDashboardData")]
        public async Task<IActionResult> FetchDashboardData([FromBody] UserIdDTO data)
        {
            var response = new APIResponse();

            try
            {
                var propertyCount = await _iProperties.FetchTotalPropertiesCount(data.userId);

                response.Result = new { propertyCount } ;
                response.IsSuccess = true;
                response.Messages = new List<string> { "success" };
                return Ok(response);
            }
